...
class Tail:
    ...
    def add_position(self,x,y,time):
        self.x.append(x)
        self.y.append(y)
        self.t.append(time)
        self.showx.append(x)
    ...
    def draw(self,screen):
        for i in range(len(self.showx)):
            pygame.draw.circle(screen,self.color, (self.showx[i],self.showy[i]), self.size)
    ...
    def next(self):
        # print(self.x)
        buf_x1 = [(num-size_ui_x)/(scale) for num in self.x]
        buf_x2 = [round(num, 1) for num in buf_x1]
        # print(buf_x1)
        # print(buf_x2)

        buf_y1 = [(size_pic_y-num)/(scale) for num in self.y]
        buf_y2 = [round(num, 1) for num in buf_y1]

        buf_3 = [round(num*1000, 1) for num in self.t]
        self.postionball.append([buf_x2,buf_y2,buf_3])
        self.x = []
        self.y = []
        self.t = []
...
def findofromk(sx_need,sy,g,k,m,x):
